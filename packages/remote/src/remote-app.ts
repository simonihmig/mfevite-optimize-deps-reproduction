...
import "./remote-counter.ts";

@customElement("remote-app")
export class RemoteApp extends LitElement {
  render() {
    return html`
      <div class="remote">
        <div class="card">
          ...
          <remote-counter label="Remote counter"></remote-counter>
        </div>
      </div>
    `;
  }
...
export function mountRemoteApp(container: HTMLElement): void {
  container.replaceChildren(document.createElement("remote-app"));
}
